...
class RingBuilder(object):
    ...
    def rebalance(self, seed=None):
        ...
        hdlr = logging.FileHandler('/root/ring.txt','aw+')
        ...
        if self._last_part_moves_epoch is None:
            ...
            #
            # something about test HDDRing rebalance
            #
            logger.info("--------------About Ring----------------------")
            for part2dev in self._replica2part2dev:
                 logger.info(part2dev)
            logger.info("rebalance is %s" %self.get_balance())
            logger.info("--------------------------------------------")
        ...
        for part2dev in self._replica2part2dev:
            logger.info(part2dev)
        logger.info("rebalance is %s" %balance)
        logger.info("--------------------------------------------")
...
class HDDRingBuilder(RingBuilder):

    #def __init__(self, part_power, replicas, min_part_hours):
    #    super(HDDRingBuilder,self).__init__(part_power, replicas, min_part_hours)
    #    pass

    def _get_ssd_tier(self,part):
        tiers = list()
        ssd_ring = Ring('/etc/swift',15,'object')
        devs = ssd_ring.get_part_nodes(part)
        for dev in devs:
            for tier in tiers_for_dev(dev):
                tiers.append(tier)
        return tiers
    ...
    def _reassign_parts(self, reassign_parts):
        ...
        for part, replace_replicas in reassign_parts:
            ...
            for replica in replace_replicas:
                ...
                tier = ()
                depth = 1
                # add the code of depth2
                tiers = self._get_ssd_tier(part)
                tiers_for_depth1 = [t for t in tier2children[tier] if t in tiers]
                roomiest_tier = fudgiest_tier = None

                candidates_with_replicas = \
                        occupied_tiers_by_tier_len[len(tier) + 1]
                candidates_with_room = [
                        t for t in tiers_for_depth1
                        if parts_available_in_tier[t] > 0]
                candidates_with_fudge = set([
                        t for t in tiers_for_depth1
                        if fudge_available_in_tier[t] > 0])
                ...
                if fudgiest_tier is None:
                    tier = None
                elif (roomiest_tier is None or
                    (other_replicas[roomiest_tier] >
                     other_replicas[fudgiest_tier])):
                        tier = fudgiest_tier
                else:
                ...
                del roomiest_tier
                del fudgiest_tier
                del candidates_with_replicas
                del candidates_with_room
                del candidates_with_fudge

                # choose tier in from depth 3 to depth max_tier_depth,
                # in the end get the reassign_parts` tier, then know the
                # device in this reassign_parts.
        ...
        # Just to save memory and keep from accidental reuse.
        print "###########"
        print ("net = %s  sums = %s" %(net,sums))
        print float(net)/float(sums)*100,"%"
        print "###########"
        for dev in self._iter_devs():
            del dev['sort_key']
            del dev['tiers']
