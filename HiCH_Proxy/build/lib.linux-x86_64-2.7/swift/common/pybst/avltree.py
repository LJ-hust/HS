...
class AVLNode(Node):
    ...
    def __init__(self,key,capacity,time,path_info):
...
class AVLTree(BSTree):
    ...
    def __init__(self,*args):
        ...
        self.tpath = "/root/metaTree.txt"
        self.counts = 0

    def load_tree(self):
        try:
            metaTree = pickle.load(open(self.tpath,"r"))
        except:
            metaTree = AVLTree()
        return metaTree

    def save(self,avl):
        pickle.dump(avl,open(self.tpath,"w"))
    ...
    def get_node(self,key,*args):
        ...
        """
        node = BSTree.get_node(self,key,*args)
        return node

    def insert(self,key,capacity,time,path_info,*args):
        """
        ...
        if not isinstance(key,(int,long,float)):
            ...
            if not self.Root:
                self.counts += 1
                self.Root = AVLNode(key,capacity,time,path_info)
            elif len(args) == 0:
                if not self.get_node(key,self.Root):
                        self.insert(key,capacity,time,path_info,self.Root)
            else:
                self.counts += 1
                child = AVLNode(key,capacity,time,path_info)
                parent = args[0]
                if child.key > parent.key:
                    if not parent.right:
                        ...
                        self.insert(key,capacity,time,path_info,parent.right)
                else:
                    if not parent.left:
                        parent.left = child
                        child.parent = parent
                        ...
                        self.insert(key,capacity,time,path_info,parent.left)
    ...
    def insert_node(self,node,*args):
        """
        T.insert(key,value...) <==> T[key] = value. Inserts
        a new Node with key attribute key and value attribute
        value into T. Balances if necessary.
        """
        if not isinstance(node.key,(int,long,float)):
            raise TypeError(str(key) + " is not a number")
        else:
            if not self.Root:
                self.counts += 1
                ...
                if not self.get_node(node.key,self.Root):
