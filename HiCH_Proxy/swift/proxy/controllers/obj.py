...
class BaseObjectController(Controller):
    ...
    def GET(self, req):
        ...
        try:
            if req.headers['Flag']:
                del req.headers['Flag']
                resp_bak =  self.GETorHEAD(req)
                metaTree = AVLTree().load_tree()
                capacity = int(resp_bak.headers['Content-Length'])
                key = int(md5(''+req.environ['PATH_INFO']+'').hexdigest(),16)
                now = time.time()
                if capacity:
                    oth_policy = POLICIES.get_by_index(1)
                    ...
                    res = self._reroute(oth_policy).GET_PUT(req, resp_bak)
                    print '---------------PUT OK ?-------------------------'
                    print res.status_int
                    metaTree.save(metaTree)
                node = metaTree.get_node(key)
                if node:
                    req.headers['Flag'] = True
                    del req.headers['X-Copy-From']
                    req.path_info = '/v1/%s/%s/%s' % \
                            (self.account_name, self.container_name, self.object_name)
                    req.headers['X-Backend-Storage-Policy-Index'] = policy_index
                    req.method = 'DELETE'
                    ress = self._reroute(policy).DELETE_NODE(req)
                    print '---------------DELETE OK ?-------------------------'
                    print ress.status_int
                    del req.headers['Flag']
        except:
            pass
        return resp
    ...
    def REPSTH(self, req):
        ...
        if node:
            ...
            #req.headers.pop('X-Backend-Storage-Policy-Index', None)
            req.headers['X-Backend-Storage-Policy-Index'] = 1
            req.headers['X-Newest'] = 'true'
            req.method = 'GET'
            resp = self._reroute(policy).GET(req)
            print '---------------GET OK ?-------------------------'
            print resp.status_int
            dest_policy = POLICIES.get_by_index(0)
            req.headers['X-Backend-Storage-Policy-Index'] = 0
            req.method = 'PUT'
            req.headers['Content-Length'] = 0
            req.headers['hot-flag'] = 'cold'
            ps = self._reroute(dest_policy).GET_PUT(req,resp)
            print '---------------PUT OK ?-------------------------'
            print ps.status_int
            ...
            return HTTPPreconditionFailed(request=req,
                                          body='NO this SSD datas\n')

    def GET_PUT(self, req, resp):
        ...
        if source_header:
            error_response, req, data_source, update_response = \
                self._handle_getput_request(req, resp)
            if error_response:
                return error_response
    ...
    def _handle_getput_request(self, req, resp):
        """
        ...
        source_resp = hook(req, source_resp, sink_req)
    ...
    def PUT(self, req):
        req.headers['X-Backend-Storage-Policy-Index'] = 1
        req.headers['hot-flag'] = 'hot'
        policy_index = 1
        policy = POLICIES.get_by_index(policy_index)
        res_a = self._reroute(policy).PUT_ONE(req)
        print '-------------res_a-------------------'
        print res_a.status_int
        source = '/%s/%s' % (self.container_name, self.object_name)
        req.headers['Content-Length'] = 0
        req.headers['X-Copy-From'] = quote(source)
        req.headers['X-Newest'] = 'true'
        ...
        req.headers['Content-Length'] = 0
        req.headers['hot-flag'] = 'hot'
        res_b = self._reroute(dest_policy).GET_PUT(req,resp)
        print '-------------res_b-------------------'
        print res_b.status_int
        return res_b

    def PUT_ONE(self, req):
        """HTTP PUT request handler."""
        if req.if_none_match is not None and '*' not in req.if_none_match:
            # Sending an etag with if-none-match isn't currently supported
            return HTTPBadRequest(request=req, content_type='text/plain',
    ...
    def DELETE(self, req):
        ...
        policy_index = 0
        policy = POLICIES.get_by_index(policy_index)
        res_a = self._reroute(policy).DELETE_ONE(req)
        try:
            if req.headers['Two']:
                del req.headers['Two']
                req.headers['X-Backend-Storage-Policy-Index'] = 1
                policy_index = 1
                policy = POLICIES.get_by_index(policy_index)
                res_b = self._reroute(policy).DELETE_ONE(req)
        except:
            pass
        return res_a
