...
class BaseObjectController(Controller):
    ...
    def GET(self, req):
        ...
        if resp.headers.get('REPHTS',None) is not None:
            self.REPHTS(req)
        return resp
        ...
        try:
            if req.headers['Flag']:
                ...
                now = time.time()
                if capacity < 5000:
                    oth_policy = POLICIES.get_by_index(0)
                ...
                if node:
                    ...
                    req.headers['X-Backend-Storage-Policy-Index'] = policy_index
                    req.method = 'DELETE'
                    ress = self._reroute(policy).DELETE(req)
                    print '---------------DELETE OK ?-------------------------'
    ...
    def REPHTS(self, req):
        ...
        req.method = 'GET'
        resp_bak =  self.GETorHEAD(req)
        print resp_bak.status_int
        if resp_bak.status_int == 200:
            metaTree = AVLTree().load_tree()
            capacity = int(resp_bak.headers['Content-Length'])
            key = int(md5(''+req.environ['PATH_INFO']+'').hexdigest(),16)
            now = time.time()
    ...
    def REPSTH(self, req):
        ...
        if node:
            ...
            req.headers['X-Backend-Storage-Policy'] = 0
            req.headers['X-Newest'] = 'true'
            req.method = 'GET'
            resp = self.GET(req)
            print '---------------GET OK ?-------------------------'
            print resp.status_int
            dest_policy = POLICIES.get_by_index(2)
    ...
    def PUT(self, req):
        ...
        req.headers['hot-flag']  = req.headers.get('hot-flag', 'cold')
